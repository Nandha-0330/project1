<!doctype html>
<html lang="en">
<meta http-equiv="content-type" content="text/html;charset=UTF-8" />

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=5" />
    <title>  {{$app_name}} {{$type}} (Fast and Unlimited) | Jeranyt Services</title>
    <link rel="icon" href="img/favicon.png">
    <link rel="preconnect" href="https://cdn.jsdelivr.net/" crossorigin="anonymous">
    <meta name="description" content="Do you want   Instagram Followers? Receive fast,   and unlimited Instagram followers with  DEMO." />
    <link href="{{ asset('assets/css/form.css') }}" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css" integrity="sha256-9kPW/n5nn53j4WMRYAxe9c1rCY96Oogo/MKSVdKzPmI=" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>

    <div class="modern-checkout">

        <section class="modern-checkout__header">
            <div class="header__inner">
                <a class="header__logo" >
                    <img src="img/prime-likes-logo.png" draggable="false" width="220" />
                </a>
            </div>
        </section>

        <section class="modern-checkout__body">


            @include('pages.form.insta_form')
         <!-- form -->
                <div class="body__card card-step card-select">
                    <h3>After one of our other social media services instead?</h3>
                    <br>
                    <h4>Please select one below:</h4>
                    <div class="message message--red">
                        <div class="message__title">
                            <p></p>
                            <img class="message__close" src="img/cross-black.svg" draggable="false" alt="Black X icon" />
                        </div>
                        <div class="message__body">
                            <p></p>
                        </div>
                    </div>
                    <br>
                    <div class="form__element form__element--with-left-icon form__element--select">
                        <h3>
                            <i class="bi bi-instagram"></i> Instagram <i class="bi bi-instagram"></i>
                        </h3>
                        <a href=" -instagram-likes.html">
                            <button class="large-button large-button--with-loader">
                                <p>Instagram Likes</p>
                            </button>
                        </a>
                        <br>
                        <a href=" -instagram-followers.html">
                            <button class="large-button large-button--with-loader">
                                <p>Instagram Followers</p>
                            </button>
                        </a>
                        <br>
                        <a href=" -instagram-views.html">
                            <button class="large-button large-button--with-loader">
                                <p>Instagram Views</p>
                            </button>
                        </a>
                        <br>
                        {{-- <h3>
                            <i class="bi bi-tiktok"></i> TikTok <i class="bi bi-tiktok"></i>
                        </h3>
                        <a href=" -tiktok-followers.html">
                            <button class="large-button large-button--with-loader">
                                <p>  TikTok Followers</p>
                            </button>
                        </a>
                        <br>
                        <a href=" -tiktok-likes.html">
                            <button class="large-button large-button--with-loader">
                                <p>  TikTok Likes</p>
                            </button>
                        </a>
                        <br>
                        <a href=" -tiktok-views.html">
                            <button class="large-button large-button--with-loader">
                                <p>  TikTok Views</p>
                            </button>
                        </a>
                        <br>
                        <h3>
                            <i class="bi bi-twitter"></i> Twitter <i class="bi bi-twitter"></i>
                        </h3>
                        <a href=" -twitter-followers.html">
                            <button class="large-button large-button--with-loader">
                                <p>  Twitter Followers</p>
                            </button>
                        </a>
                        <br>
                        <a href=" -twitter-likes.html">
                            <button class="large-button large-button--with-loader">
                                <p>  Twitter Likes</p>
                            </button>
                        </a>
                        <br>
                        <a href=" -twitter-retweets.html">
                            <button class="large-button large-button--with-loader">
                                <p>  Twitter Retweets</p>
                            </button>
                        </a>
                        <br> --}}
                        <h3>
                            <i class="bi bi-facebook"></i> Facebook <i class="bi bi-facebook"></i>
                        </h3>
                        <button class="large-button large-button--with-loader">
                            <a href=" -facebook-followers.html">
                                <p>Facebook Followers</p>
                            </a>
                        </button>
                        <br>
                        <a href=" -facebook-likes.html">
                            <button class="large-button large-button--with-loader">
                                <p>Facebook Likes</p>
                            </button>
                        </a>
                        <br>
                        {{-- <h3>
                            <i class="bi bi-instagram"></i> Threads <i class="bi bi-instagram"></i>
                        </h3>
                        <button class="large-button large-button--with-loader">
                            <a href=" -threads-followers.html">
                                <p>  Threads Followers</p>
                            </a>
                        </button>
                        <br>
                        <a href=" -threads-likes.html">
                            <button class="large-button large-button--with-loader">
                                <p>  Threads Likes</p>
                            </button>
                        </a>
                        <br> --}}
                    </div>
                </div>
        </section>
    </div>
</html>
